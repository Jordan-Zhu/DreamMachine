...
{
	symbolTable[symbPointer] = malloc(sizeof(symbol));

	// type = const
	if(kind == 1)
	{
		symbolTable[symbPointer].kind = 1;
		symbolTable[symbPointer].name = buff[tokPointer - 2];
		symbolTable[symbPointer].val = atoi(buff[tokPointer]);
		symbolTable[symbPointer].level = level;
		symbolTable[symbPointer].addr = m;
	}
	// type = var
	else if(kind == 2)
	{
		symbolTable[symbPointer].kind = 2;
		symbolTable[symbPointer].name = buff[tokPointer];
		symbolTable[symbPointer].level = level;
		symbolTable[symbPointer].addr = m;
	}
	// type = proc
